...
if (!function_exists('home_page')) {
    function home_page($key) {
        return HomePage::where('key', $key)->first();
    }
}


function callAPI($method, $url, $header = null, $json = false)
{
    $client = new  Client();
    try {
        $res = $client->request($method, $url, $header ? $header : []);
